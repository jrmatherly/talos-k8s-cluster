...
30. **K8s NetworkPolicy policyTypes: [Egress] blocks ALL egress** - Kubernetes NetworkPolicy with `policyTypes: [Egress]` blocks ALL egress except what's explicitly allowed; use CiliumNetworkPolicy instead
31. **CiliumNetworkPolicy egress creates IMPLICIT default-deny** - When CNP has egress rules, it blocks ALL OTHER egress from selected endpoints! Must include ALL destinations: intra-namespace, DNS, world, kube-apiserver. See `docs/ai-context/flux-web-ui.md` for complete example
