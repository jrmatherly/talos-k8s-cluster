...
**Why both steps?** `task configure` renders Jinja2 templates including `talconfig.yaml`, but does NOT run `talhelper genconfig`. The individual node config files in `talos/clusterconfig/` are only regenerated by `task talos:generate-config`.
...
When adding new application templates to this project, multiple files must be updated **before** running `task configure`. See `.serena/memories/adding-new-templates-checklist.md` for the complete checklist.
